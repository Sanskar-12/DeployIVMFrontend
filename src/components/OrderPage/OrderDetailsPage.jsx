...
const OrderDetailsPage = () => {
  ...
  useEffect(() => {
    axios
      .get(`${server}/order/getById/${orderId}`,{
  ...
  useEffect(() => {
    if (order) {
      ...
      axios
        .get(`${server}/vendor/get-vendor-byId/${vid}`,{
          withCredentials: true,
        })
        .then((response) => {
    ...
    <div className='flex flex-row items-start justify-between'>
      <div className='flex flex-col bg-white p-6 m-4 w-1/4  border-2  border-[#E0E2E7] rounded-md'>
        <div>
          ...
          <div className="text-lg font-semibold flex  items-center ">Order {order._id.substring(0, 6)}</div>
          <div className="bg-[#FDF1E8] rounded-full h-auto w-auto  text-center text-sm p-2 text-[#E46A11] font-extrabold">{order.orderStatus}</div>

          </div>
          <div className='flex flex-row justify-between mt-2 '> 
            <div className='mt-6 '>
            <span className='bg-[#E0E2E7] p-2 rounded-full'><StorefrontIcon/></span>
            </div>
            <div className='p-2 font-medium text-md'>{order.department}</div>
          </div>
          <div className='flex flex-row justify-between mt-2 py-6'> 
            <div><span className='bg-[#E0E2E7] p-2 rounded-full'><CreditCardIcon /></span>
            <span className='p-2  font-medium text-md'>{order.itemtype}</span></div>
          </div>
          <div className='flex flex-row justify-between mt-2 py-6'> 
            <div><span className='bg-[#E0E2E7] p-2 rounded-full'><EventAvailableIcon /></span>
            <span className='p-2  font-medium text-md'>Ordered</span></div>
            <div className='mr-6'>{order.createdBy.substring(0,10)}</div>
          </div>
        </div>
      </div>
      <div className='flex flex-col bg-white p-6 m-4 w-1/4  border-2   border-[#E0E2E7] rounded-md'>
        <div>
          ...
          <div className='flex flex-col justify-between mt-4 w-full'> 
            <div className='py-4'>
            <span className='bg-[#E0E2E7] p-2 rounded-full'><FmdGoodIcon /></span>
            <span className='p-2  font-medium text-md'>Location : {order.lab}</span>
            </div>
            <div className='py-4'>
            ...
            <span className='bg-[#E0E2E7] p-2 rounded-full'><LocalShippingIcon /></span>
            <span className='p-2  font-medium text-md'>Initiator : {order.requisition_name} </span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div className='flex flex-col'> 
            <div className='py-6 flex flex-row justify-between '>
              ...
              <div className='p-2'>{vendor.name}</div>
            </div>
            <div className='py-6 flex flex-row justify-between'>
              <div className='flex flex-row'>
             ...
             <div className='p-2'>{vendor.contact} </div>
            </div>
            <div className='py-6 flex flex-row justify-between'>
              <div className='flex flex-row'>
    ...
    <tbody className='text-center'>
      {Products.map((data, index) => (
        <tr key={index} className="bg-white  border-[#E0E2E7] border-b">
          ...
          <td className="px-6 py-4 text-[#5C59E8] font-bold">{data.name}</td>
