...
import {
  deletePurchaseOrderDataAction,
  getAllPurchaseOrderDataAction,
...
const PurchaseOrderList = () => {
  ...
  const { getpurchaseOrderdata, loading } = useSelector(
    (state) => state.purchaseOrders
  );

  useEffect(() => {
    dispatch(getAllPurchaseOrderDataAction());
  }, [dispatch]);

  const handleDelete = async (id) => {
    await dispatch(deletePurchaseOrderDataAction(id));
    dispatch(getAllPurchaseOrderDataAction());
  };
  ...
  const handleClick = () => {
    ...
    const tableRows = selectedRows.map(
      (item) =>
        ...
        <td>${item?.general_Information?.to}</td>
        <td>${
  ...
  const generatePDF = () => {
    ...
    pdf.text(20, 40, `Subject: ${getworkOrderdata?.general_Information?.to}`);
    pdf.text(
      20,
      50,
      `Subject: ${getworkOrderdata?.general_Information?.subject}`
  ...
  const handlePreview = (id) => {
    const fetchData = async () => {
      await axios
        .get(`/api/v1/get/purchaseorder/${id}`, {
        ...
        <>
          <div>
            <div>
              ...
              {/* top bar inward */}
              <div>
                <div className="text-[24px]">Purchase Order List</div>
                <div className="flex">
                  <Link
                    to="/home"
              ...
              <div className="block bg-transparent m-4 p-4 overflow-x-auto w-full ">
                <table className="w-full shadow-md  border-2 rounded-2xl">
                  ...
                  <tbody className="bg-white">
                    {getpurchaseOrderdata?.map((order) => (
                      <tr key={order?._id}>
